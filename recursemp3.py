...
class RecurseMP3:
    def __init__(self):
        ...
        self.mainlogger = self.logger_config.getLogger("recursemp3")
    ...
    def display_progress(self, i_number_of_files_processed, i_number_of_files_to_process, i_step=10, last_line=False):
        b_step_reached = bool((i_number_of_files_processed % i_step) == 0)

        if b_step_reached ^ last_line:
            self.mainlogger.info("Processed " + str(i_number_of_files_processed) + " of " + str(
    ...
    def process(self):
        ...
        i_number_of_files_to_process = len(self.mp3_files)
        self.display_progress(i_number_of_files_processed, i_number_of_files_to_process)
        if i_number_of_files_to_process == 0:
            self.mainlogger.warning("No files to process")
            sys.exit(0)

        # SEARCH #
        for current_file in self.mp3_files:
            ...
            if not self.cover_only:
                ...
                m_track_and_disk_index_found = re.match(self.p_find_track_and_disk_information, filename_without_extension)
        ...
        self.display_progress(i_number_of_files_processed, i_number_of_files_to_process, last_line=True)
